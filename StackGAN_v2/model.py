...
class MY_Inception3(models.Inception3):
    def __init__(self, num_classes=1000, aux_logits=True, transform_input=False):
        super(MY_Inception3, self).__init__(num_classes, aux_logits, transform_input)
    ...
    def forward(self, x):
        ...
        x = self.Mixed_7c(x)
        # 8 x 8 x 2048
        x_pool_3 = F.avg_pool2d(x, kernel_size=8)
        # 1 x 1 x 2048
        x = F.dropout(x_pool_3, training=self.training)
        # 1 x 1 x 2048
        x = x.view(x.size(0), -1)
        # 2048
        x = self.fc(x)
        ...
        if self.training and self.aux_logits:
            return x, aux
        return x, x_pool_3.squeeze()


class INCEPTION_V3(nn.Module):
    def __init__(self):
        super(INCEPTION_V3, self).__init__()
        self.model = MY_Inception3()
...
class GLU(nn.Module):
    ...
    def forward(self, x):
        ...
        nc = int(nc/2)
        x_temp, x_temp1 = x[:, :nc].clone(), x[:, nc:].clone()
        rtn = x_temp * torch.sigmoid(x_temp1)
        return rtn
...
class CA_NET(nn.Module):
    ...
    def reparametrize(self, mu, logvar):
        std = logvar.mul(0.5).exp_().requires_grad_(True)
        if cfg.CUDA:
            eps = torch.cuda.FloatTensor(std.size()).normal_().requires_grad_(True)
        else:
            eps = torch.FloatTensor(std.size()).normal_().requires_grad_(True)
        eps = (eps)
...
class INIT_STAGE_G(nn.Module):
    ...
    def forward(self, z_code, c_code=None):
        if cfg.GAN.B_CONDITION and c_code is not None:
            in_code = torch.cat((c_code.clone(), z_code.clone()), 1)
        else:
            in_code = z_code
...
class NEXT_STAGE_G(nn.Module):
    ...
    def forward(self, h_code, c_code):
        ...
        h_c_code = torch.cat((c_code.clone(), h_code.clone()), 1)
...
class D_NET64(nn.Module):
    ...
    def forward(self, x_var, c_code=None):
        ...
        x_immediate = x_code.reshape((batch_size, -1))

        if cfg.GAN.B_CONDITION and c_code is not None:
            c_code = c_code.view(-1, self.ef_dim, 1, 1)
            c_code = c_code.repeat(1, 1, 4, 4).clone()
            # state size (ngf+egf) x 4 x 4
            h_c_code_tmp = torch.cat((c_code.clone(), x_code.clone()), dim=1)
            # state size ngf x in_size x in_size
            h_c_code = self.jointConv(h_c_code_tmp)
        else:
            h_c_code = x_code
        ...
        if cfg.GAN.B_CONDITION:
            ...
            return [output.view(-1), out_uncond.view(-1)], x_immediate
        else:
            return [output.view(-1)], x_immediate
...
class D_NET128(nn.Module):
    ...
    def forward(self, x_var, c_code=None):
        ...
        if cfg.GAN.B_CONDITION and c_code is not None:
            ...
            # state size (ngf+egf) x 4 x 4
            h_c_code = torch.cat((c_code.clone(), x_code.clone()), dim=1)
            # state size ngf x in_size x in_size
...
class D_NET256(nn.Module):
    ...
    def forward(self, x_var, c_code=None):
        ...
        if cfg.GAN.B_CONDITION and c_code is not None:
            ...
            c_code = c_code.repeat(1, 1, 4, 4)
            # state size (ngf+egf) x 4 x 4
            h_c_code = torch.cat((c_code.clone(), x_code.clone()), 1)
            # state size ngf x in_size x in_size
            h_c_code = self.jointConv(h_c_code)
        else:
            h_c_code = x_code
