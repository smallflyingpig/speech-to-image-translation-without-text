...
def parse_args():
    ...
    parser.add_argument('--my_dataset', action="store_true", default=False, help='use my own data set or not')
    parser.add_argument('--asr_flag', action="store_true", default=False, help='use audio asr embedding feature or not')
    parser.add_argument("--audio_switch", type=str, default="",help="audio swtich to be extracted")        
    parser.add_argument("--output_dir", type=str, default='')
    parser.add_argument("--distributed", action="store_true", default=False, help="")
    parser.add_argument("--eval_net_g", type=str, default='', help="path for net G when eval")
    parser.add_argument("--branch_num", type=int, default=3, help="brunch num for stackGAN v2")
...
if __name__ == "__main__":
    ...
    if args.output_dir=='':
        ...
        output_dir = './output/StackGAN_v2/%s_%s_%s' % \
            (cfg.DATASET_NAME, cfg.CONFIG_NAME, timestamp)
    else:
        output_dir = './output/StackGAN_v2/%s' % \
            (args.output_dir)
    ...
    elif cfg.DATA_DIR.find('Places') != -1:
        from datasets import PlacesSubSet
        dataset = PlacesSubSet(
            cfg.DATA_DIR, train=cfg.TRAIN.FLAG, 
            base_size=cfg.TREE.BASE_SIZE,
            transform=image_transform,
            feature_switch=args.audio_switch
            )
    elif cfg.DATA_DIR.find('flowers') != -1:
        from datasets import FlowersDataset
        dataset = FlowersDataset(cfg.DATA_DIR, train=cfg.TRAIN.FLAG,
            base_size=cfg.TREE.BASE_SIZE,
            transform=image_transform,
            feature_switch=args.audio_switch
            )
    ...
    param = {"num_workers":min(int(cfg.WORKERS), os.cpu_count()), "pin_memory":True} if cfg.CUDA else {}
    if args.distributed:
        train_sampler = torch.utils.data.distributed.DistributedSampler(dataset)
        dataloader = torch.utils.data.DataLoader(
            dataset, batch_size=cfg.TRAIN.BATCH_SIZE, 
            sampler=train_sampler, **param)
    else:
        dataloader = torch.utils.data.DataLoader(
            dataset, batch_size=cfg.TRAIN.BATCH_SIZE, 
            shuffle=True, **param)
    ...
    # output the args and cfg
    if not os.path.exists(output_dir):
        os.makedirs(output_dir, exist_ok=True)
    with open(os.path.join(output_dir, "args.txt"), "w") as fp:
        pprint.pprint(args, fp)
        pprint.pprint(cfg, fp)

    algo = trainer(output_dir, dataloader, imsize, args.my_dataset, local_rank=args.local_rank, distributed=args.distributed)
    ...
    if cfg.TRAIN.FLAG:
        ...
        # eval
        if args.eval_net_g != '':
            cfg.TRAIN.NET_G = args.eval_net_g
        fid_data = algo.evaluate(split_dir)
        # save
        with open(os.path.join(os.path.dirname(cfg.TRAIN.NET_G), 'fid_data0.np'), "wb") as fp:
            np.save(fp, fid_data[0])
        with open(os.path.join(os.path.dirname(cfg.TRAIN.NET_G), 'fid_data1.np'), "wb") as fp:
            np.save(fp, fid_data[1])
    end_t = time.time()
